    ...
    <div className="p-8 space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Platform performance analytics</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={fetchReports} className="btn-primary text-sm py-1.5 px-4">Apply</button>
        </div>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
              <h3 className="text-white font-semibold mb-4 text-sm">Daily completed sessions</h3>
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={data.daily}>
                  ...
                  <Line type="monotone" dataKey="count" stroke="#6366f1" strokeWidth={2} dot={{ r: 3, fill: '#6366f1' }} name="Sessions" />
                  <Line type="monotone" dataKey="avgScore" stroke="#22c55e" strokeWidth={2} dot={{ r: 3, fill: '#22c55e' }} name="Avg score" />
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
              <h3 className="text-white font-semibold mb-4 text-sm">Emotion distribution</h3>
              {emotionData.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={emotionData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={({ name, percent }) => `${name} ${(percent*100).toFixed(0)}%`} labelLine={false}>
                ...
                <div className="h-48 flex items-center justify-center text-gray-600 text-sm">No emotion data</div>
              )}
            </div>
          </div>
              ...
              <div className="space-y-2">
                {data.topPerformers.map((p, i) => (
                  <div key={i} className="flex items-center gap-4 py-2 border-b border-gray-800 last:border-0">
                    <span className={`text-lg font-bold w-6 text-center ${i === 0 ? 'text-amber-400' : i === 1 ? 'text-gray-300' : i === 2 ? 'text-amber-700' : 'text-gray-600'}`}>
                      {i + 1}
                    </span>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-800">
              <h3 className="text-white font-semibold text-sm">Recent completed sessions</h3>
            </div>
                ...
                {data.sessions.slice(0, 20).map(s => (
                  <tr key={s._id} className="hover:bg-gray-800/50 transition">
                    <td className="px-4 py-3">
                      <p className="text-sm text-white">{s.userId?.name || 'Unknown'}</p>
                      <p className="text-xs text-gray-500">{s.userId?.email}</p>
                    </td>
                    <td className="px-4 py-3 text-xs text-gray-400">{s.techStack?.slice(0,3).join(', ')}</td>
                    <td className="px-4 py-3">
